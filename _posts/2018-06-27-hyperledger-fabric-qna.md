...
A. Kafka queue에 들어간 순서로 ordering 되며, 큐에는 시간(Orderer에 들어온 시간) 순서대로 쌓이게 됩니다. 추가로 큐는 개별 채널에 의존하여 만들어집니다. 
...
A. Kafka 큐에는 시간 순서대로 쌓이게 되고, 이후 처리는 Kafka에 의해서 처리되기 때문에 순서가 보장됩니다. 다만 Orderer에 들어오기 전에는 Fabric 네트워크 내 참여자 간의 Consistency를 보장하기 위해서 Applicatoin이 이를 보장하도록 설계되어야 합니다. 또한, Fabric을 일반 DB 처럼 사용하기보다는 공유 원장으로서 사용, DB로 처리해야 할것들은 DB로 처리한 후에 원장에 해당하는 데이터만 쌓도록 설계하는것이 좋습니다.
```
...
A. 네, 따라서 Membership service와 orderer는 HA를 보장하도록 구성해야 합니다. Production 환경에서 Kafka 브로커는 9개, Zookeeper는 5개를 두는 것이 권장됩니다.
```

```
Q. Commiter에 의한 Validation 과정에서 블록에 포함된 Transaction 중 하나라도 Validation에 실패하게 되면 전체 Transaction이 다 실패하나요?
...
```
Q. Application이 Transaction propose 할 때 endorsing peer가 하나 이상이라면 모든 peer에 다 propose 하나요?

A. Endorsement policy에 의해 반드시 서명을 받아야 하는 peer 리스트에 있는 peer 에게 모두 Proposal을 보내게 됩니다.
```

```
Q. Chaincode 없이 Couch DB에 접근이 가능한데, World State가 위변조 되었다고 의심될 때, Blockchain을 이용해서 World State를 replay 하는 기능이 있나요?

A. 없습니다. 다만, 제공되는 kafka image를 보면 retention policy를 영구 저장으로 설정하고 있기 때문에 위변조가 의심되는 상황에서 kafka 데이터를 확인 할 수 있습니다. Couch DB 또한 ACL 기능을 가지고 있기 때문에 정책적으로 접근을 관리 할 수 있습니다.
...
```
Q. Chaincode로 구현되어야 하는 내용들은 어떤것들이 될까요?

A. Chaincode는 Database의 Procedure에 대응합니다. 따라서 Chaincode로는 Business logic이 구현되어야 합니다. 추가로 원장의 데이터에 대한 접근을 관리하는 기능을 포함할 수 있습니다.
```

```
Q. Fabric 네트워크 운영 중 Org 또는 channel의 추가 등 Fabric network의 설정을 변경 할 수 있나요? 
...
개발 모드에서는 chaincode container에서 chaincode를 직접 빌드하여 실행하지만
...
A. Orderer로 SBFT를 사용하게 되는경우 Transaction 검증은 Peer에서 하고 Orderer는 SBFT를 이용하여 Transaction을 정렬하게 할 수 있습니다.
```
...
Instantiate는 chaincode 별로 한 번만 수행하면 되기 때문에 예제를 수행하는 과정에서 Instantiate 까지만 수행하면 chaincode container가 하나만 구동되는 것 처럼 보일 수 있습니다. 이후에 다른 peer에서 query나 invoke Transaction이 실행되면  chaincode container peer와 짝을 이루는 chaincode container가 실행되게 됩니다.
```
